...
export default function TechnologyCard({ technology, isSelected, handleAddStack }: TechnologyCardPropType) {
    return (
        <div className={`flex flex-col justify-center gap-1.5 border border-[#F1F5F9] p-2 m-1 rounded-2xl ${isSelected ? 'border-2 border-gray-300' : ''} `} >
            ...
            <button onClick={() => {
                ...
                className={`rounded-lg px-4.5 py-2.5 transition-colors ${isSelected
                    ? 'text-red-500 bg-gray-300  cursor-not-allowed font-bold'
